...
DT = 1 / 50
CALIB_SAMPLES = 100
BETA_VALUES = [0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0]
RAD2DEG = 180.0 / np.pi
...
def run_beta_test(mux, beta, duration_s, gyro_bias):
...
def save_plot(path, rows):
    ...
    ax1 = axes[0]
    ax1.plot(betas, lat_max, marker="o", color="tab:red", label="Max latency")
    ax1.set_title("Max Latency vs Beta")
    ax1.set_xlabel("Beta")
    ...
    ax2 = axes[1]
    ax2.plot(betas, lat_mean, marker="o", color="tab:blue", label="Mean latency")
    ax2.set_title("Mean Latency vs Beta")
    ax2.set_xlabel("Beta")
...
def main():
    parser = argparse.ArgumentParser(description="Sweep Madgwick beta values and plot latency/frequency.")
    parser.add_argument("--duration", type=float, default=30.0, help="Test duration per beta in seconds")
    parser.add_argument("--csv", default="beta_sweep_results.csv", help="Output CSV path")
    parser.add_argument("--plot", default="beta_sweep_plot.png", help="Output plot path")
    args = parser.parse_args()

    # Fail fast on missing filter dependency before touching the hardware loop.
    require_ahrs()
    ...
    for beta in BETA_VALUES:
        print(f"Running beta={beta:.2f} for {args.duration:.0f}s...")
        row = run_beta_test(mux, beta, args.duration, gyro_bias)
        results.append(row)
        print(
            f"beta={row['beta']:.2f} | mean={row['lat_mean_ms']:.2f} ms | "
            f"min/max={row['lat_min_ms']:.2f}/{row['lat_max_ms']:.2f} ms | "
